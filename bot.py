...
from config import (
    ...
    LEVERAGE, RISK_PER_TRADE,
    MAX_OPEN_POSITIONS, SCAN_INTERVAL, CAPITAL_LIMIT,
    TOP_SYMBOLS_COUNT
)
from strategy import calculate_signals, get_dynamic_sl_tp, format_telegram_message
from telegram_notify import notify_start, notify_close, notify_error, _send
...
TIMEFRAME = "15"  # 15-минутные свечи (Каворк рекомендует)
...
def get_candles(client: HTTP, symbol: str) -> list:
    r = client.get_kline(category=CATEGORY, symbol=symbol, interval=TIMEFRAME, limit=200)
...
def place_order(client: HTTP, symbol: str, sig: dict, balance: float):
    side = sig["signal"]
    price = sig["price"]
    atr_val = sig["atr"]
    ...
    sl_raw, tp_raw = get_dynamic_sl_tp(price, atr_val, side)
    sl = round_step(sl_raw, tick)
    tp = round_step(tp_raw, tick)
    ...
    try:
        ...
        log.info(
            f"ОРДЕР {side} {symbol} | qty={qty_str} | цена={price} | "
            f"SL={sl} | TP={tp} | ADX={sig['adx']} | id={order_id}"
        )
        # Красивое Telegram уведомление от Каворка
        tg_text = format_telegram_message(symbol, sig, float(sl), float(tp))
        _send(tg_text)
...
def run():
    log.info(f"=== TRADE-BOT ЗАПУЩЕН | ДЕМО={DEMO_MODE} | ТОП-{TOP_SYMBOLS_COUNT} пар | x{LEVERAGE} | 15m ===")
    client = get_client()
    ...
    while True:
        try:
            ...
            log.info(f"Баланс={balance:.2f} USDT | Позиций={open_count}/{MAX_OPEN_POSITIONS}")
                ...
                try:
                    ...
                    if sig["signal"]:
                        log.info(
                            ...
                            f"Supertrend={sig['supertrend']} | Vol={sig['volume_ratio']}x"
                        )
                        set_leverage(client, symbol)
                        place_order(client, symbol, sig, balance)
                        signals_found += 1
                    else:
                        log.debug(f"{symbol} | strength={sig['signal_strength']}/4 | {sig['reason']}")
